...
# 清理数据目录（如果存在）
[ -d "/pcdn_data/pcdn_index_data" ] && rm -rf /pcdn_data/pcdn_index_data/* 2>/dev/null
for dir in /pcdn_data/storage*_ssd; do
...
# 清理Docker容器和镜像（如果Docker已安装）
if command -v docker >/dev/null 2>&1; then
    # 停止所有运行中的容器
    running_containers=$(docker ps -q 2>/dev/null)
    [ -n "$running_containers" ] && docker stop $running_containers >/dev/null 2>&1
    
    # 删除所有容器
    all_containers=$(docker ps -aq 2>/dev/null)
    [ -n "$all_containers" ] && docker rm $all_containers >/dev/null 2>&1
    
    # 删除所有镜像
    all_images=$(docker images -q 2>/dev/null)
    [ -n "$all_images" ] && docker rmi $all_images >/dev/null 2>&1 || true
fi

# 清理挂载服务（如果存在）
shopt -s nullglob  # 设置nullglob，没有匹配时扩展为空
for service in /etc/systemd/system/pcdn_data-*.mount; do
    service_name=$(basename "$service")
    systemctl stop "$service_name" >/dev/null 2>&1 || true
    systemctl disable "$service_name" >/dev/null 2>&1 || true
    rm -f "$service"
done
shopt -u nullglob  # 恢复原设置
...
mount_points=$(mount | grep '/pcdn_data/' | awk '{print $3}' 2>/dev/null)

if mountpoint -q "/pcdn_data/pcdn_index_data" 2>/dev/null && ! check_index_on_system_disk; then
    umount "/pcdn_data/pcdn_index_data" >/dev/null 2>&1
fi
